...
hello <- function(who, lang = "EN", LangData = Hello::language) {
  if (!exists("who", mode = "character") | length(who) > 1) {
    #who karakter türünde değilse veya birden fazla değer varsa hata verir.
  ...
  if ((mode(LangData$code) != "character") | (mode(LangData$hello) != "character")) {
    #Her iki sütunun da character türünde olduğundan emin olur.
    stop("Please enter a valid LangData; see ?hello")
  }

  # Kullanıcının dil kodunu küçük harfe çevir
  llang <- tolower(lang)

  # Kullanıcının istediği dile uygun selamlamayı bul
